(ns opticalmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [opticalmfg.registry :as r]))
...
;; ----------------------------- instrument-class-valid? -----------------------------

(deftest known-instrument-classes-are-valid
  (doseq [c [:optical-lens :binocular :microscope :telescope :camera :projector]]
    (is (r/instrument-class-valid? c))))

(deftest fabricated-instrument-class-is-invalid
  (is (not (r/instrument-class-valid? :periscope)))
  (is (not (r/instrument-class-valid? nil))))

;; ----------------------------- resolution-test-line-pairs-per-mm-valid? -----------------------------

(deftest typical-resolution-test-reading-is-valid
  (is (r/resolution-test-line-pairs-per-mm-valid? 0.1))
  (is (r/resolution-test-line-pairs-per-mm-valid? 180.0))
  (is (r/resolution-test-line-pairs-per-mm-valid? 1200.0)))

(deftest below-floor-resolution-test-reading-is-invalid
  (is (not (r/resolution-test-line-pairs-per-mm-valid? 0.0)))
  (is (not (r/resolution-test-line-pairs-per-mm-valid? -1.0))))

(deftest excessive-resolution-test-reading-is-invalid
  (is (not (r/resolution-test-line-pairs-per-mm-valid? 9999.0)))
  (is (not (r/resolution-test-line-pairs-per-mm-valid? 1200.01))))

(deftest non-numeric-or-missing-resolution-test-reading-is-invalid
  (is (not (r/resolution-test-line-pairs-per-mm-valid? nil)))
  (is (not (r/resolution-test-line-pairs-per-mm-valid? "180"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "grinder-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "grinder-001" 1)
