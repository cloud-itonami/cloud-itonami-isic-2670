(ns opticalmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control lens-grinding/optics-assembly/
  testing equipment directly... does NOT authorize or execute
  maintenance/equipment operations... does NOT self-issue an IEC
  60825-1 laser safety class certification') implemented faithfully.
  The single invariant under test:

    OpticalInstrAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Optical Instrument Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:instrument-class :camera}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :camera (:instrument-class (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "grinder-001" :maintenance-type :tool-inspection
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-grinder :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "alignment-bench-002" :maintenance-type :collimation-check
                             :scheduled-date "2026-08-01" :actuate-equipment? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
          ...
          res (exec-op actor "t6"
                    {:op :coordinate-shipment :effect :propose :subject "ship-2"
                     :value {:batch-id "batch-003" :units 100.0
                             :destination "buyer-optics-distributor-south"}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
          ...
          res (exec-op actor "t7"
                    {:op :coordinate-shipment :effect :propose :subject "ship-3"
                     :value {:batch-id "batch-002" :units 100.0
                             :destination "buyer-optics-distributor-east"}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "grinder-001" :maintenance-type :force-run
...
(deftest laser-classification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-laser-safety-classification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the IEC 60825-1 laser safety classification authority"
    (let [[db actor] (fresh)
          res (exec-op actor "t8b"
                    {:op :log-production-batch :effect :propose :subject "batch-001"
                     :patch {:issue-laser-safety-classification? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:laser-classification-authority-blocked} (-> (store/ledger db) last :basis)))
      (is (not (true? (:issue-laser-safety-classification? (store/batch db "batch-001"))))
          "fabricated self-classification never lands in the SSoT"))))
...
(deftest invalid-instrument-class-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:instrument-class :periscope}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-instrument-class} (-> (store/ledger db) last :basis)))
    (is (not= :periscope (:instrument-class (store/batch db "batch-001"))) "fabricated instrument-class never lands in the SSoT")))

(deftest invalid-resolution-test-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:resolution-test-line-pairs-per-mm 9999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-resolution-test-line-pairs-per-mm} (-> (store/ledger db) last :basis)))
    (is (not= 9999.0 (:resolution-test-line-pairs-per-mm (store/batch db "batch-001"))) "fabricated resolution-test reading never lands in the SSoT")))
  ...
  (testing "flag-safety-concern always escalates -- never auto-committed, regardless of confidence"
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "grinder-001" :severity :moderate
                                            :description "laser-alignment anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "grinder-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t14" {:op :coordinate-shipment :effect :propose :subject "ship-1"
                                    :value {:batch-id "batch-001" :units 500.0
                                            :destination "buyer-optics-distributor-north"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
