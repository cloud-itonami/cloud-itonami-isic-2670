(ns opticalmfg.operation-test
  "Smoke tests for the compiled OpticalOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `opticalmfg.governor-contract-test`; the Store contract in
  `opticalmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "OpticalOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "grinder-001" :severity :moderate :description "laser-alignment anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
