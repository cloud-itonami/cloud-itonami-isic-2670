(ns opticalmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE lens-grinding/
  optics-assembly/testing equipment directly (permanently blocked, no
  override), a proposal that tries to self-issue an IEC 60825-1 laser
  safety class certification (permanently blocked, no override), a
  double-schedule of the same maintenance window, a production-batch
  patch with a fabricated instrument class, a production-batch patch
  with an implausible resolution-test reading, and a production-batch
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:instrument-class :camera :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on grinder-001 (verified, registered, lens grinder -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on grinder-001 (always escalates -- approve) ==")
    (let [r (exec-op actor "t3"
                      {:op :flag-safety-concern :effect :propose :subject "concern-1"
                       :value {:equipment-id "grinder-001" :severity :moderate
                               :description "レンズ研磨公差の逸脱兆候、レーザーアライメント再確認要"}}
                      coordinator)]
      (println r)
      (println "-- human plant supervisor approves --")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:instrument-class :camera}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-grinder :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on alignment-bench-002 (UNVERIFIED/unregistered test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on grinder-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated instrument-class -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:instrument-class :periscope}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible resolution-test reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:resolution-test-line-pairs-per-mm 9999.0}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue an IEC 60825-1 laser safety classification -> HARD hold, PERMANENT ==")
