(ns opticalmfg.store
  "SSoT for the optical-instrument-and-photographic-equipment plant-
  operations coordination actor, behind a `Store` protocol so the
  backend is a swap, not a rewrite -- the same seam every
  `cloud-itonami-isic-*` actor in this fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-3250`'s own
  `medinstrmfg.store`, `cloud-itonami-isic-2652`'s own
  `watchmfg.store`), this build ships a single `MemStore` backend only
  ...
  self-contained (no external optical-instrument-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             instrument-class/resolution-test/quantity/
    ...
    - `equipment`         -- a lens-grinding/optics-assembly/testing
    ...
    - `maintenance`       -- a scheduled lens-grinding/optics-assembly/
                             testing-equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`opticalmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound optical-instrument/
                             photographic-equipment shipment DRAFT
                             (`opticalmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (opticalmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (opticalmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :instrument-class :camera
                :lot-number "OPT-2026-001" :resolution-test-line-pairs-per-mm 180.0
                :quantity-units 5000.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :instrument-class :binocular
                :lot-number "OPT-2026-002" :resolution-test-line-pairs-per-mm 95.0
                :quantity-units 800.0 :defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 750.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :instrument-class :microscope
                :lot-number "OPT-2026-003" :resolution-test-line-pairs-per-mm 620.0
...
(defn- sample-equipment []
  {"grinder-001" {:id "grinder-001" :kind :lens-grinder
                   :verified? true :registered? true
                   :last-maintenance-date "2026-05-01"}
   "alignment-bench-002" {:id "alignment-bench-002" :kind :laser-alignment-test-bench
                            :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `opticalmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `opticalmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  lens-grinder unit (schedulable for maintenance), one
  UNVERIFIED/unregistered laser-alignment-test-bench unit (blocks any
