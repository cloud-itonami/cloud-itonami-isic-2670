(ns opticalmfg.advisor
  "OpticalInstrAdvisor -- the *contained intelligence node* for the
  optical-instrument-and-photographic-equipment plant-operations
  coordination actor.

  It normalizes production-batch patches (instrument-class/resolution-
  test/quantity/defect-rate), drafts a lens-grinding/optics-assembly/
  testing-equipment maintenance scheduling proposal against a piece of
  equipment, drafts a safety-concern flag, and drafts an outbound
  optical-instrument/photographic-equipment shipment coordination
  ...
  NEVER a real lens-grinding/optics-assembly/testing-equipment
  actuation, freight dispatch, or IEC 60825-1 laser safety class
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `opticalmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct lens-
                                 ; grinding/optics-assembly-line-
  ...
  `opticalmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [opticalmfg.registry :as registry]
            [opticalmfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's instrument-
  class, resolution-test reading, quantity, or defect-rate, nor its
...
(defn- schedule-maintenance
  "Draft a lens-grinding/optics-assembly/testing-equipment maintenance-
  ...
  rationale, but `opticalmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a materials-safety/precision-defect/laser-alignment-hazard
  ...
  technicality). See `opticalmfg.phase`: no phase ever adds this op to
  a phase's `:auto` set; `opticalmfg.governor` also always escalates
...
(defn- coordinate-shipment
  "Draft an outbound optical-instrument/photographic-equipment shipment
  ...
  one, and `opticalmfg.governor` NEVER trusts it: it independently
...
(def ^:private system-prompt
  (str "あなたは光学機器・写真機器製造工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "レンズ研磨・光学系組立・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "IEC 60825-1レーザー安全クラス認証等を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `opticalmfg.governor`
