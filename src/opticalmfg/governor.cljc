(ns opticalmfg.governor
  "Optical Instrument Plant Operations Governor -- the independent
  compliance layer that earns the OpticalInstrAdvisor the right to
  ...
  draft-schedule) lens-grinding/optics-assembly/testing equipment,
  whether a proposal secretly tries to self-issue an IEC 60825-1 laser
  safety class certification (an authority this actor never holds),
  ...
  stops being a coordination proposal and becomes direct lens-
  grinding/optics-assembly-line-equipment control, so this MUST be a
  separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:optical-instrument-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:grinder/actuate`
                                       or `:test-bench/run`) is the
                                       'direct lens-grinding/optics-
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       true`? Directly actuating lens-
                                       grinding/optics-assembly/testing
    ...
    5. Laser-safety-classification-
       authority blocked            -- ANY proposal (any op) whose own
                                       ...
                                       `:issue-laser-safety-
                                       classification? true` is
                                       attempting to self-issue an IEC
   ...
   11. Invalid resolution-test
       reading                      -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:resolution-test-line-pairs-
                                       per-mm` that is not a physically
                                       plausible resolution reading
                                       (`opticalmfg.registry/
                                       resolution-test-line-pairs-per-
                                       mm-valid?`), the batch record is
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [opticalmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct lens-grinding/
  optics-assembly-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct lens-grinding/optics-assembly-line-equipment
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はレンズ研磨・光学系組立・試験等の設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- actuate-equipment-blocked-violations
  ...
  to directly actuate lens-grinding/optics-assembly/testing equipment
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :actuate-equipment-blocked
      :detail "レンズ研磨/光学系組立/試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))

(defn- laser-classification-authority-blocked-violations
  "HARD, PERMANENT, unconditional: ANY proposal (any op) whose own
  `:value`/`:patch` declares `:issue-laser-safety-classification?
  true` is attempting to self-issue an IEC 60825-1 laser safety class
  certification -- an authority exclusively reserved to the accredited
  testing laboratory, never this actor. No phase and no human approval
  can ever override this."
  [proposal]
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-laser-safety-classification? payload))
      [{:rule :laser-classification-authority-blocked
        :detail "IEC 60825-1レーザー安全クラス認証の自己発行提案は恒久的に禁止 -- 認定試験機関の専権事項"}])))
...
(defn- invalid-resolution-test-violations
  "For `:log-production-batch`, if the patch declares a
  `:resolution-test-line-pairs-per-mm` that is not a physically
  plausible resolution reading, reject rather than let fabricated/
  sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [lp (:resolution-test-line-pairs-per-mm (:value proposal))]
      (when (and (some? lp) (not (registry/resolution-test-line-pairs-per-mm-valid? lp)))
        [{:rule :invalid-resolution-test-line-pairs-per-mm
          :detail (str lp "lp/mm は物理的に妥当な解像度試験結果の範囲外")}]))))
...
(defn check
  "Censors an OpticalInstrAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (laser-classification-authority-blocked-violations proposal)
                           ...
                           (invalid-resolution-test-violations request proposal)
